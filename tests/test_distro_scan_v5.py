"""Wave 6 — the v5 build added NO new native/third-party import.

Proves IMPLEMENTATION-PLAN.md "## Wave 6 — ... distro scan": after Waves 1-5,
`distro.py`'s stdlib-only import scan still reports ONLY the declared, file-scoped
`pywinpty` exception. No v5 module leaks an undeclared third-party/native import,
and — critically — **Playwright never appears in product code** (it is a DEV-ONLY
test dep, imported via `pytest.importorskip` in tests only).
...
def test_v5_real_product_import_scan_is_clean():
    ...
    assert hits == [], f"undeclared third-party import(s) after v5: {hits}"


def test_v5_full_scan_clean():
...
def test_only_declared_pywinpty_exception():
    ...
    assert set(allow.keys()) == {"winpty"}, (
        "no v5 wave may add a new third-party-import exception")
    assert allow["winpty"]["files"] == frozenset({"pty_manager.py"})
    assert distro._import_allowed("winpty", "pty_manager.py")


def test_v5_modules_scan_clean_individually():
    """Each v5-touched product module scans clean (no native import leak)."""
    v5_modules = [
        "terminal_session.py", "pty_manager.py", "summarizer.py",
        "deliverables.py", "effort_history.py", "session_registry.py",
        "anchor_gui.py", "anchor.py", "anchor_healthcheck.py",
    ]
    selected = set(distro.select_shippable())
    for mod in v5_modules:
