...
def test_grass_column_renders_in_window(gui, tmp_path):
    ...
    so 'Open workbench' still opens it. Since the 2026-08-07 simple-workbench
    cut, the classic escape hatch shows one General-session zone; trio work is
    commissioned through Steward rather than hand-started from lane columns."""
    folder = tmp_path / "P"
    folder.mkdir()
    ...
    # The one General zone + the Deliverables panel still render. Retired trio
    # launch zones must not quietly return on the classic fallback page.
    assert html.count("class='sectionlbl'") == 1
    assert "Latest General session" in html
    assert "Latest Research" not in html
    assert "Latest Plan" not in html
...
def test_most_recent_plus_previous_sessions_expander(gui, tmp_path):
    ...
    # The simple-workbench contract exposes General sessions on this page.
    ...
    # The two General sessions land in the General zone: ONE prominent headline +
    ...
    assert "Show all 1 older general sessions" in board, \
        "no General shelf toggle for the older session (collapsed default)"
    ...
    # Exactly two General-session tiles: one headline and one minitile.
    general_tiles = re.findall(
        r"<div class='(?:headline|minitile) tile lane-tile[^>]*"
        r"data-lane=\"general\"", board)
    assert len(general_tiles) == 2, \
        "expected exactly 2 General session tiles, got %d" % len(general_tiles)
    shelf_at = board.find("id='shelf_general'")
    prominent = board[:shelf_at]
    shelf = board[shelf_at:]
    assert newer["session_id"] in prominent, "newest General session is not the headline"
    assert older["session_id"] in shelf, "older General session is not in the shelf"
...
def test_linked_tasks_strip_renders_with_link_affordance(gui, tmp_path):
    """Link/add-to-task-list controls are gone from the project window
    (2026-08-28). The home Tasks tile is the editor."""
    folder = tmp_path / "P"
    folder.mkdir()
    pid = gui.select_existing_project("Linker", str(folder))["entry"]["id"]
    html = gui.render_project_window_html(pid)
    assert "+ Link task" not in html
    assert "Use the Tasks tile on the dashboard" in html


def test_linked_tasks_strip_reflects_a_linked_task(gui, tmp_path):
    """A task carrying ``Project: <id>`` still lists on the strip, without Unlink."""
    folder = tmp_path / "P"
    folder.mkdir()
...
def test_session_card_inline_expand_wiring_not_navigation(gui, tmp_path):
    ...
    import session_registry as reg
    reg.register_session(pid, "general", status=reg.STATUS_RUNNING)
    html = gui.render_project_window_html(pid)
    ...
    assert "function _loadPanelSummary" in js
    ...
    assert "/api/rnd/session_summary" in js
...
def test_launch_new_session_button_wired_to_wave4_path(gui, tmp_path):
    ...
    assert "/api/rnd/term_start" in html
    # The surviving live start affordance is General only. Research/plan/build
    # are commissioned through Steward rather than hand-started here.
...
def test_linked_task_with_quote_and_apostrophe_roundtrips(gui, tmp_path):
    ...
    # (a) Render: text appears HTML-escaped (link/unlink controls were dropped).
    html = gui.render_project_tasks_html(pid)
    assert "rndUnlinkTask(" not in html
    assert html_lib.escape(task_text) in html
    assert "Fix it's" not in html  # apostrophe must be escaped in the markup
