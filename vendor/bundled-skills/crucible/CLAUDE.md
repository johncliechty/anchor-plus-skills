...
Orientation for any coding-agent session opened in `<path> Read first.
...
- `plans/research/round-2/` — Lane E (brownfield ingest), Lane F (persistent Deep-Think). Round-1 research: `<path>
- `CLAUDE_hist.md` — dated history.

## Building Crucible (the Foreman run)
The plan is converged + Foreman-ready (passes `node <path> --json .`). Build harness: **`<path> — binds `agent()` to headless `claude -p` on the subscription (no API key); git ON, branch `foreman/run`, never pushes; containment-safe (crucible is a sibling of foreman). Commands:
- `node <path> --smoke` — wiring check (passed 2026-06-05).
- `node <path> --max-waves=1` — **proving wave** (Wave 1 only); recommended first.
- `node <path> --resume` — continue remaining waves.
- `node <path> — full autonomous (all 11 waves).

Live progress → `<path> Code lands in `bin/` + `test/` on branch `foreman/run`; Foreman commits per GREEN wave. Halts on blocker/budget/Pro-window → re-run with `--resume`. NOTE: launching an autonomous agent loop requires an explicit human go — an agent cannot self-launch it (the auto-mode classifier blocks that); run it via the `!` prefix or grant a Bash permission rule.
