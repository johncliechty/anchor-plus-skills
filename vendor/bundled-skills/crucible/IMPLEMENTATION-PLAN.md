...
- **Given** a managed repo whose toplevel would either contain OR be nested inside `<path> **when** the git context initializes, **then** it isolates (separate worktree/copy) instead of letting `assertContainment` HALT.
